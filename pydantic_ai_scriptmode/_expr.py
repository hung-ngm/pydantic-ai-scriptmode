...
def holds_function_value(value: Any) -> bool:
    """Whether `value` is, or contains, a lambda or builtin reference rather than data.

    Such a value cannot be recorded or returned. Walks lists and dict values, which the budget
    already bounded when they were built.
    """
    if isinstance(value, (_Closure, _Builtin)):
        return True
    if isinstance(value, list):
        return any(holds_function_value(v) for v in value)  # pyright: ignore[reportUnknownVariableType]
    if isinstance(value, dict):
        return any(holds_function_value(v) for v in value.values())  # pyright: ignore[reportUnknownVariableType]
    return False
